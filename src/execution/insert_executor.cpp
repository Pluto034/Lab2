 ...
 * insert into t1 values (0, '🥰', 10), (1, '🥰🥰', 11), (2, '🥰🥰🥰', 12), (3, '🥰🥰🥰🥰', 13), (5,
 * '🥰🥰🥰🥰🥰', 14);
 */
  ...
  if (insert_item_ != nullptr) {
    ...
    for (; insert_item_->Next(&tmp_tuple, &tmp_rid); insert_cnt++) {
      ...
      for (auto index_info_ : indexes) {
        ...
        htable_->InsertEntry(tmp_tuple.KeyFromTuple(table->schema_, *htable_->GetKeySchema(), htable_->GetKeyAttrs()),
                             res_rid.value(), exec_ctx_->GetTransaction());
      }
    }
